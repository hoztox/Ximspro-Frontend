...
const AddCarNumberModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const fetchNextActionNumber = async () => {
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}/qms/car-number/next-action/${companyId}/`
      );
      if (response.data && response.data.next_action_no) {
        // Changed from next_action_number to next_action_no
  ...
  const handleSubmit = async (e, asDraft = false) => {
    ...
    try {
      ...
      setSuccessMessage("CAR Number Added Successfully");
      // onClose();
    } catch (error) {
  ...
  const handleDraftSave = async (e) => {
    ...
    try {
      ...
      setSuccessMessage("CAR Number Drafted Successfully");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300 overflow-y-scroll">
      <div
        className={`bg-[#1C1C24] text-white rounded-[4px] w-[1014px] p-5 transform transition-all duration-300 ${animateClass}`}
      >
