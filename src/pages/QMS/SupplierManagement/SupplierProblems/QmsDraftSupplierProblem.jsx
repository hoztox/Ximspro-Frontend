...
const QmsDraftSupplierProblem = () => {
  ...
  const [deleteMessage, setDeleteMessage] = useState("Draft Supplier Problem");
    ...
    const fetchDraftSupplierProblems = async () => {
      try {
        ...
        const response = await axios.get(
          `${BASE_URL}/qms/supplier-problems-draft/${companyId}/`
        );
        console.log("Draft Supplier Problems:", response.data);

        const draftProblems = response.data.filter(
  ...
  const filteredSuppliers = currentItems.filter((supplier) => {
    ...
    const carMatch =
      supplier.no_car &&
      typeof supplier.no_car.action_no === "string" &&
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setSuccessMessage("Draft supplier problem deleted successfully");
      setTimeout(() => {
    ...
    return (
      <div className="bg-[#1C1C24] p-5 rounded-lg">
        <div className="flex justify-center items-center h-64">
          <p className="not-found">Loading draft supplier problems...</p>
        </div>
      </div>
    );
  }
            ...
            {filteredSuppliers.length > 0 ? (
              filteredSuppliers.map((supplier, index) => (
                ...
                >
                  ...
                  <td className="px-2 add-manual-datas !text-left">
                    <button
                      onClick={() =>
                        handleQmsEditDraftSupplierProblem(supplier.id)
                      }
                  ...
                  <td className="px-2 add-manual-datas !text-center">
                    <button
                      onClick={() =>
                        handleQmsViewDraftSupplierProblem(supplier.id)
                      }
                    >
                      <img
                        src={viewIcon}
              ...
              <tr className="border-b border-[#383840] h-[50px]">
                <td colSpan="7" className="text-center not-found">
                  {searchQuery
                    ? "No matching draft supplier problems found"
                    : "No draft supplier problems available"}
                </td>
              </tr>
            )}
          </tbody>
