...
const QmsEditSupplierProblem = () => {
        ...
        const fetchSupplierProblem = async () => {
            try {
                ...
                setFormData({
                    ...
                    solved: typeof problemData.solved === 'boolean' 
                        ? (problemData.solved ? 'Yes' : 'No') 
                        : (problemData.solved === 'Yes' ? 'Yes' : 'No'),
                ...
                setError("Failed to load supplier problem data. Please check your connection and try again.");
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(`${BASE_URL}/qms/supplier-problems/${id}/`, finalSubmissionData);

            // Navigate back to list on success
            navigate('/company/qms/supplier-problem-log');
        } catch (err) {
            console.error('Error updating supplier problem:', err);
            setError(`Failed to update supplier problem: ${err.response?.data?.solved || 'Please try again.'}`);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    if (loading) {
        return <div className="bg-[#1C1C24] text-white p-5 rounded-lg flex justify-center items-center h-64">Loading supplier problem data...</div>;
    }

    if (error) {
        return <div className="bg-[#1C1C24] text-white p-5 rounded-lg flex justify-center items-center h-64 text-red-500">{error}</div>;
    }
