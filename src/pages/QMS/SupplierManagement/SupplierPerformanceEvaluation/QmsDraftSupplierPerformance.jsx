...
const QmsDraftSupplierPerformance = () => {
    ...
    const fetchEvaluationData = async () => {
      try {
        ...
        const response = await axios.get(
          `${BASE_URL}/qms/supplier/evaluation-draft/${id}/`
        );

        // Filter evaluations where is_draft is true
        const draftEvaluations = response.data.filter(
        ...
        console.error("Error fetching draft supplier evaluation data:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleView = (id) => {
    navigate(`/company/qms/views-drafts-supplier-evaluation/${id}`);
  };

  // Edit supplier
  const handleEditDraft = (id) => {
    navigate(`/company/qms/edits-drafts-supplier-evaluation/${id}`);
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setEvaluationData(
        evaluationData.filter((item) => item.id !== evaluationToDelete.id)
      );
  ...
  const currentItems = filteredEvaluationData.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  return (
    <div className="bg-[#1C1C24] text-white p-5 rounded-lg">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center pb-5">
        <h1 className="employee-performance-head">Draft Supplier Evaluation</h1>
        ...
        <div className="flex w-full md:w-auto gap-4">
          ...
          <button
            ...
            onClick={handleCloseDraftSupplierEvaluation}
          >
            ...
            <tbody>
              {currentItems.length > 0 ? (
                ...
                <tr className="border-b border-[#383840]">
                  <td colSpan="7" className="py-4 text-center not-found">
                    No draft supplier evaluations found
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {!loading && !error && filteredEvaluationData.length > 0 && (
        <div className="flex justify-between items-center mt-3">
          <div className="text-white total-text">
            Total-{filteredEvaluationData.length}
          </div>
