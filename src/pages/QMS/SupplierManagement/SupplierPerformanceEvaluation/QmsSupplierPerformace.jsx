...
const EvaluationModal = ({
  ...
  const [selectedSupplier, setSelectedSupplier] = useState(
    supplier ? supplier.id || "Select Supplier" : "Select Supplier"
  );
  ...
  const fetchQuestions = async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_URL}/qms/supplier/evaluation/${evaluationId}/questions/`
      );
  ...
  const handleSubmitAllAnswers = async () => {
    ...
    try {
      ...
      for (const question of questionsWithAnswers) {
        await axios.patch(
          `${BASE_URL}/qms/supplier/evaluation/question/answer/${question.id}/`,
          {
  ...
  const combinedOptions = [...(supplierList || []), ...(users || [])].reduce(
    (unique, item) => {
              ...
              <div className="w-[528px]">
                ...
                <div className="p-5 pt-6">
                  <div className="flex relative items-center gap-3">
                    ...
                    <div className="absolute -top-[9px] right-[158px] flex items-center pr-2 pointer-events-none mt-6">
                      <ChevronDown
                        size={20}
...
const QuestionsModal = ({ isOpen, onClose, evaluationId }) => {
  const navigate = useNavigate();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}/qms/supplier/evaluation/question-add/`,
        {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(
        `${BASE_URL}/qms/supplier/evaluation/question/${questionToDelete.id}/delete/`
      );
...
const QmsSupplierPerformace = () => {
  ...
  // Fetch supplier evaluation data
  useEffect(() => {
    const fetchEvaluationData = async () => {
      setLoading(true);
      try {
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `${BASE_URL}/qms/supplier/evaluation/${evaluationToDelete.id}/update/`
      );
      setEvaluations(
        evaluations.filter(
          (evaluation) => evaluation.id !== evaluationToDelete.id
        )
      );
      setShowDeleteModal(false);
  ...
  const openQuestionsModal = (evaluationId) => {
    setQuestionsModal({ isOpen: true, evaluationId });
  };
  ...
  return (
    <div className="bg-[#1C1C24] text-white p-5 rounded-lg">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center pb-5">
        <h1 className="employee-performance-head">List Supplier Evaluation</h1>
        ...
        <div className="flex w-full md:w-auto gap-4">
          ...
          <button
            ...
            onClick={handleDraftSupplierEvaluation}
          >
            <span>Draft</span>
            {draftCount > 0 && (
              <span className="bg-red-500 text-white rounded-full text-xs flex justify-center items-center w-[20px] h-[20px] absolute top-[115px] right-[232px]">
                {draftCount}
              </span>
            )}
          </button>
          ...
          <button
            ...
            onClick={handleAddSupplierEvaluation}
          >
            <span>Add Supplier Evaluation</span>
            <img
              src={plusIcon}
              alt="Add Icon"
              ...
              currentItems.map((evaluation, index) => (
                ...
                >
                  ...
                  <td className="px-2 add-manual-datas">
                    {evaluation.title || "Anonymous"}
                  </td>
                  <td className="px-2 add-manual-datas">
                    {formatDate(evaluation.valid_till)}
                  </td>
                  <td className="px-2 add-manual-datas">
                    <button
                  ...
                  <td className="px-2 add-manual-datas">
                    <button
                      className="text-[#1E84AF] hover:text-[#176d8e] transition-colors"
                      onClick={() => openQuestionsModal(evaluation.id)}
                    >
                      Add Questions
                    </button>
                  </td>
                  ...
                  <td className="px-2 add-manual-datas !text-center">
                    <button onClick={() => openDeleteModal(evaluation)}>
                      <img
                        src={deleteIcon}
                        alt="Delete Icon"
                        className="action-btn"
      ...
      <EvaluationModal
        ...
        onClose={() => setEvaluationModal({ isOpen: false, supplier: null })}
        supplier={evaluationModal.supplier}
        supplierList={suppliers}
        evaluationId={evaluationModal.evaluationId}
      />

      <QuestionsModal
        isOpen={questionsModal.isOpen}
        onClose={() => setQuestionsModal({ isOpen: false })}
        evaluationId={questionsModal.evaluationId}
      />
...
export default QmsSupplierPerformace;
