...
const QmsEditComplaints = () => {
  ...
  const fetchCategories = async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_URL}/qms/category/company/${companyId}/`
      );
  ...
  const handleChange = (e) => {
    ...
    if (name.startsWith("date.")) {
      ...
      if (
        updatedDateInputs.day &&
        updatedDateInputs.month &&
        updatedDateInputs.year
      ) {
  ...
  const handleCloseModal = () => {
    ...
    fetchCategories(); // Add this line to refresh categories
  };
    ...
    <div className="bg-[#1C1C24] text-white p-5 rounded-lg">
      ...
      ${focusedDropdown === "customer" ? "rotate-180" : ""}`}
        ...
        <div className="flex flex-col gap-3">
          <label className="add-training-label">Details</label>
          <textarea
            name="details"
        ...
        <div className="flex flex-col gap-3 relative">
          ...
          <ChevronDown
            className={`absolute right-3 top-[60%] transform transition-transform duration-300 
                             ${
                               focusedDropdown === "solved_after_action"
                                 ? "rotate-180"
                                 : ""
                             }`}
            size={20}
