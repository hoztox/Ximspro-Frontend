...
const InternalProblemsModal = ({ isOpen, onClose }) => {
    ...
    const [causesItems, setCausesItems] = useState([
    ...
    const [newCausesTitle, setNewCausesTitle] = useState("");
    const [isAddingCauses, setIsAddingCauses] = useState(false);
    const [nextId, setNextId] = useState(5);

    const handleDelete = (id) => {
        setCausesItems(causesItems.filter(item => item.id !== id));
    };

    const handleSave = () => {
        if (newCausesTitle.trim()) {
            setCausesItems([...causesItems, { id: nextId, title: newCausesTitle }]);
            setNextId(nextId + 1);
            setNewCausesTitle("");
            setIsAddingCauses(false);
        }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300">
            <div className={`bg-[#13131A] text-white rounded-[4px] w-[563px] p-5 transform transition-all duration-300 ${animateClass}`}>
                ...
                <div className="bg-[#1C1C24] rounded-[4px]">
                    <h3 className="agenda-list-head border-b border-[#383840] px-5 py-6">Add Causes</h3>
                    ...
                    <div className="mb-4 px-5">
                        ...
                        <input
                            type="text"
                            value={newCausesTitle}
                            onChange={(e) => setNewCausesTitle(e.target.value)}
