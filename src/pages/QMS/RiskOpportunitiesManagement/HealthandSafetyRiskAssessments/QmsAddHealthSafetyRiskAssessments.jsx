...
const QmsAddHealthSafetyRiskAssessments = () => {
  ...
  const [showAddManualSuccessModal, setShowAddManualSuccessModal] =
    useState(false);
  const [showDraftManualSuccessModal, setShowDraftManualSuccessModal] =
    useState(false);
  ...
  const fetchUsers = async () => {
    try {
      ...
      setError(
        "Failed to load health safety risk assessments. Please check your connection and try again."
      );
    }
  };
  ...
  const validateForm = () => {
    ...
    if (
      formData.review_frequency_year &&
      isNaN(parseInt(formData.review_frequency_year))
    ) {
      newErrors.review_frequency_year = "Year must be a number";
    }

    if (
      formData.review_frequency_month &&
      isNaN(parseInt(formData.review_frequency_month))
    ) {
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      console.log("Risk Assessment post ...................", formData);
      ...
      const response = await axios.post(
        `${BASE_URL}/qms/assessment-create/`,
        submitData,
        {
          headers: {
  ...
  const handleDraftClick = async () => {
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/qms/assessment/draft-create/`,
        submitData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      <div>
        <div className="flex items-center justify-between  px-[65px] 2xl:px-[122px]">
          <h1 className="add-manual-sections !px-0">
            Add Health and Safety Risk Assessments
          </h1>
...
export default QmsAddHealthSafetyRiskAssessments;
