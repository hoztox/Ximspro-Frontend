...
const QmsListHealthSafetyHazards = () => {
    ...
    const fetchAllData = async () => {
      try {
        ...
        const correctionsPromises = filteredManuals.map(async (manual) => {
          try {
            ...
            const correctionResponse = await axios.get(
              `${BASE_URL}/qms/health/${manual.id}/corrections/`
            );
            console.log(
              `Corrections for hazards ${manual.id}:`,
            ...
            console.error(
              `Error fetching corrections for hazards ${manual.id}:`,
              correctionError
            );
        ...
        const draftResponse = await axios.get(
          `${BASE_URL}/qms/health/drafts-count/${id}/`
        );
  ...
  const filteredManual = manuals.filter(
    (manual) =>
      ...
      (manual.hazard_source?.toLowerCase() || "").includes(
        searchQuery.toLowerCase()
      ) ||
      (manual.hazards_no?.toLowerCase() || "").includes(
        searchQuery.toLowerCase()
      ) ||
      (manual.approved_by?.first_name?.toLowerCase() || "").includes(
        searchQuery.toLowerCase()
      ) ||
      (manual.level_of_risk?.toLowerCase() || "").includes(
        searchQuery.toLowerCase()
      ) ||
  ...
  const handleQMSAddEvaluationCompliance = () => {
    navigate("/company/qms/add-health-safety-hazards");
  };

  const handleEdit = (id) => {
    navigate(`/company/qms/edit-health-safety-hazards/${id}`);
  };
  ...
  const handlePublishSave = async () => {
    try {
      ...
      await axios.post(
        `${BASE_URL}/qms/health/${selectedManualId}/publish-notification/`,
        {
  ...
  const canReview = (manual) => {
    ...
    if (manual.status === "Correction Requested") {
      ...
      const canCorrect = manualCorrections.some(
        (correction) =>
  ...
  return (
    <div className="bg-[#1C1C24] list-manual-main">
      <div className="flex items-center justify-between px-[14px] pt-[24px]">
        <h1 className="list-manual-head">List Health and Safety Hazards</h1>
        ...
        <div className="flex space-x-5">
          ...
          >
            <span>Drafts</span>
            {draftCount > 0 && (
              <span className="bg-red-500 text-white rounded-full text-xs flex justify-center items-center w-[20px] h-[20px] absolute top-[120px] right-[288px]">
                {draftCount}
              </span>
            )}
          </button>
          ...
          >
            <span>Add Health and Safety Hazards</span>
            <img
              src={plusicon}
              alt="Add Icon"
      ...
      <div className="p-5 overflow-hidden">
        {loading ? (
          <div className="text-center py-4 not-found">
            Loading Health and Safety Hazards...
          </div>
        ) : (
            ...
            <tbody key={currentPage}>
              {paginatedManual.length > 0 ? (
                paginatedManual.map((manual, index) => {
                  ...
                  console.log(
                    `Manual ${index} (${manual.id}): can review = ${canApprove}`
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-2 add-manual-datas">
                        {manual.hazard_no || "N/A"}
                      </td>
                      <td className="px-2 add-manual-datas">
                        {manual.legal_requirement || "N/A"}
                      </td>
                      <td className="px-2 add-manual-datas">
                        {formatDate(manual.date)}
                      </td>
                      <td className="px-2 add-manual-datas">
                        {manual.level_of_risk || "N/A"}
                      </td>
                ...
                <tr>
                  <td colSpan="11" className="text-center py-4 not-found">
                    No Health and Safety Hazards Found.
                  </td>
                </tr>
              )}
              <tr>
...
export default QmsListHealthSafetyHazards;
