...
const QmsDeleteLegalSuccessModal = ({ showDeleteLegalSuccessModal, onClose }) => {
  const { theme } = useTheme();

  if (!showDeleteLegalSuccessModal) return null;
  return (
    <AnimatePresence>
      {showDeleteLegalSuccessModal && (
        <motion.div
        ...
        >
          ...
          >
            ...
            <div className={`success-modal-content flex flex-col items-center justify-center ${theme === "dark" ? "dark" : "light"}`}>
              ...
              <p className="success-messege">Legal/Requirements Deleted Successfully</p>
            </div>
          </motion.div>
        </motion.div>
      )}
...
export default QmsDeleteLegalSuccessModal
