...
const QmsSaveDraftComplianceSuccessModal = ({ showDraftComplianceSuccessModal, onClose }) => {
  const { theme } = useTheme();

  if (!showDraftComplianceSuccessModal) return null;
  return (
    <AnimatePresence>
      {showDraftComplianceSuccessModal && (
        <motion.div
        ...
        >
          ...
          >
            ...
            <div className={`success-modal-content flex flex-col items-center justify-center ${theme === "dark" ? "dark" : "light"}`}>
              ...
              <p className="success-messege">Compliance Drafted Successfully</p>
            </div>
          </motion.div>
        </motion.div>
      )}
...
export default QmsSaveDraftComplianceSuccessModal
