...
const QmsListCompliance = () => {
  ...
  const fetchDraftCount = async () => {
    try {
      ...
      const draftResponse = await axios.get(
        `${BASE_URL}/qms/compliance/drafts-count/${id}/`
      );
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(
        `${BASE_URL}/qms/compliances-get/${complianceToDelete}/`
      );
      setComplianceData(
        complianceData.filter((item) => item.id !== complianceToDelete)
      );
  ...
  const currentItems = publishedCompliances.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
                  ...
                  {currentItems.length > 0 ? (
                    currentItems.map((item, index) => (
                      ...
                      >
                        ...
                        <td className="px-4 qms-list-compliance-data">
                          {item.complianceNo || "N/A"}
                        </td>
                        <td className="px-4 qms-list-compliance-data">
                          {item.complianceType || "N/A"}
                        </td>
                        <td className="px-4 qms-list-compliance-data">
                          {item.revision || "N/A"}
                        </td>
                    ...
                    <tr>
                      <td
                        colSpan="9"
                        className="px-4 py-8 text-center not-found"
                      >
