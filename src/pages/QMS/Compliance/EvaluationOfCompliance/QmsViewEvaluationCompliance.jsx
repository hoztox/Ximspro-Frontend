...
const QmsViewEvaluationCompliance = () => {
  ...
  const fetchManualDetails = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}/qms/evaluation-detail/${id}/`
      );
      setManualDetails(response.data);
      console.log("Evaluation Compliance Details:", response.data);
      setLoading(false);
    } catch (err) {
      console.error("Error fetching evaluation compliance details:", err);
  ...
  const fetchManualCorrections = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}/qms/evaluation/${id}/corrections/`
      );
      const allCorrections = response.data;
      console.log("Fetched Evaluation Compliance Corrections:", allCorrections);
      ...
      console.error("Error fetching evaluation compliance corrections:", error);
    }
  };
  ...
  const handleCorrectionSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/qms/evaluation/submit-correction/`,
        requestData
      );
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`${BASE_URL}/qms/evaluation-detail/${id}/`);
      ...
      console.error("Error deleting evaluation compliance:", err);
      setShowDeleteModal(false);
      let errorMsg = err.message;
  ...
  if (!manualDetails)
    return (
      <div className="text-center not-found">
        No evaluation compliance details found
      </div>
    );
  ...
  const handleReviewAndSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/qms/evaluation-review/`,
        requestData
      );
    ...
    <div className="bg-[#1C1C24] p-5 rounded-lg">
      <div className="flex justify-between items-center border-b border-[#383840] pb-[26px]">
        <h1 className="viewmanualhead">
          Evaluation of Compliance/Obligation Information
        </h1>
        <button
      ...
      <div className="mt-5">
        <div className="grid grid-cols-2 divide-x divide-[#383840] border-b border-[#383840] pb-5">
          <div className="grid grid-cols-1 gap-[40px]">
            <div>
              <label className="viewmanuallabels">
                  ...
                  <div className="flex flex-col justify-center items-center">
                    ...
                    <button
                      onClick={() => {
                        handleMoveToHistory();
                        navigate(
                          `/company/qms/edit-evaluation-compliance/${id}`
                        );
                      }}
                    >
...
export default QmsViewEvaluationCompliance;