    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get(`${BASE_URL}/qms/interst-draft/${id}/`);
                        ...
                        {currentRecords.length > 0 ? (
                            currentRecords.map((item, index) => (
                                <tr key={item.id} className="border-b border-[#2D2D35] h-[48px]">
                                    ...
                                    <td className="text-center">
                                        <button onClick={() => handleDelete(item.id)}><img src={deletes} alt="Edit" className="w-4 h-4" /></button>
                                    </td>
                                </tr>
                            ))
                        ) : (
                            <tr>
                                <td colSpan="7" className="text-center py-4">No draft interested parties found.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
