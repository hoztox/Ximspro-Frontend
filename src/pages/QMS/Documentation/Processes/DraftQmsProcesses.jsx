    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get(`${BASE_URL}/qms/process-draft/${id}/`);
                ...
                <table className="w-full text-sm">
                    <thead className="bg-[#24242D] text-left">
                        <tr className="h-[48px]">
                            ...
                            <th className="px-4 qms-interested-parties-thead text-left">Actions</th>
                        ...
                        {currentRecords.length > 0 ? (
                            currentRecords.map((item, index) => (
                                <tr key={item.id} className="border-b border-[#2D2D35] h-[48px]">
                                    ...
                                    <td className="px-4 qms-interested-parties-data">{item.no || "-"}</td>
                                    <td className="px-4 qms-interested-parties-data">{item.type}</td>
                                    <td className="px-4 qms-interested-parties-data">{item.created_at?.slice(0, 10)}</td>
                                    <td className="px-4 qms-interested-parties-data">
                                        <button onClick={() => handleEdit(item.id)} className="text-[#1E84AF]">
                                            Click to Continue
                                        </button>
                                    </td>
                                    <td className="text-center">
                                        <button onClick={() => handleView(item.id)}><img src={views} alt="View" className="w-4 h-4" /></button>
                                    </td>
                                    <td className="text-center">
                                        <button onClick={() => handleDelete(item.id)}><img src={deletes} alt="Delete" className="w-4 h-4" /></button>
                                    </td>
                                </tr>
                            ))
                        ) : (
                            <tr>
                                <td colSpan="8" className="text-center py-4">No draft processes found.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
