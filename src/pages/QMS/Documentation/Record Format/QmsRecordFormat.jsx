...
const QmsRecordFormat = () => {
  ...
  // State variables
  const [recordFormats, setRecordFormats] = useState(demoData);
  ...
  const fetchProcedures = () => {
    console.log("Fetching Record Formats...");
    setLoading(true);
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this Record Formats?")) {
      const updatedProcedures = recordFormats.filter(recordFormat => recordFormat.id !== id);
  ...
  const handleClickApprove = (id, newStatus) => {
    console.log(`Approving Record Formats ID: ${id} to status: ${newStatus}`);
    navigate(`/company/qms/viewprocedure/${id}`);
  };

  const handleEdit = () => {
    navigate(`/company/qms/editrecordformat`);
  };
  ...
  const handleQMSAddRecordFormat = () => {
  ...
  const handlePublish = (recordFormat) => {
    setSelectedProcedure(recordFormat);
  ...
  // Utility functions
  const canReview = (recordFormat) => {
    const currentUserId = currentUser.id;
    const recordFormatCorrections = corrections[recordFormat.id] || [];

    if (recordFormat.status === "Pending for Review/Checking") {
      return currentUserId === recordFormat.checked_by?.id;
    }

    if (recordFormat.status === "Correction Requested") {
      return recordFormatCorrections.some(correction =>
        correction.to_user?.id === currentUserId
      );
    }

    if (recordFormat.status === "Reviewed,Pending for Approval") {
      return currentUserId === recordFormat.approved_by?.id;
    }
  ...
  const totalPages = Math.ceil(filteredProcedure.length / recordFormatsPerPage);
  const paginatedProcedure = filteredProcedure.slice(
    (currentPage - 1) * recordFormatsPerPage,
    currentPage * recordFormatsPerPage
  );
  ...
  return (
    <div className="bg-[#1C1C24] list-manual-main">
      <div className="flex items-center justify-between px-[14px] pt-[24px]">
        ...
        <div className="flex space-x-5">
          ...
          >
            <span>Drafts</span>
            {draftCount > 0 && (
              <span className="bg-red-500 text-white rounded-full text-xs flex justify-center items-center w-[20px] h-[20px] absolute top-[120px] right-[215px]">
                {draftCount}
              </span>
            )}
          </button>
          <button
            ...
            onClick={handleQMSAddRecordFormat}
          >
      ...
      <div className="p-5 overflow-hidden">
        {loading ? (
          <div className="text-center py-4 text-white">Loading Record Formats...</div>
        ) : error ? (
          ...
          <table className="w-full">
            ...
            <tbody key={currentPage}>
              {paginatedProcedure.length > 0 ? (
                paginatedProcedure.map((recordFormat, index) => {
                  const canApprove = canReview(recordFormat);

                  return (
                    <tr key={recordFormat.id} className="border-b border-[#383840] hover:bg-[#1a1a20] h-[46px]">
                      <td className="pl-5 pr-2 add-manual-datas">{(currentPage - 1) * recordFormatsPerPage + index + 1}</td>
                      ...
                      <td className='px-2 add-manual-datas'>
                        {recordFormat.status === 'Approved' ? (
                          <button className="text-[#36DDAE]" onClick={() => handlePublish(recordFormat)}>Click to Publish</button>
                        ) : canApprove ? (
                          <button
                            onClick={() => handleClickApprove(recordFormat.id,
                              recordFormat.status === 'Pending for Review/Checking'
                                ? 'Reviewed,Pending for Approval'
                                : (recordFormat.status === 'Correction Requested'
                                  ? 'Approved'
                                  : 'Approved')
                            )}
                      ...
                      <td className="px-2 add-manual-datas text-center">
                        <button
                          onClick={() => handleView(recordFormat.id)}
                          title="View"
                        >
