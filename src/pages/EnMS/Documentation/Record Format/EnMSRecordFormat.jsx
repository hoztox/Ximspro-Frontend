...
const EnMSRecordFormat = () => {
    ...
    const filteredrecordFormat = recordFormat.filter(recordFormats =>
        (recordFormats.title?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
        (recordFormats.no?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
        (recordFormats.approved_by?.first_name?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
        (recordFormats.rivision?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
        (formatDate(recordFormats.date)?.replace(/^0+/, '') || '').includes(searchQuery.replace(/^0+/, ''))
    );
        ...
        <div className="bg-[#1C1C24] list-manual-main">
            <div className="flex items-center justify-between px-[14px] pt-[24px]">
                ...
                <div className="flex space-x-5">
                    ...
                    <button className="flex items-center justify-center add-manual-btn gap-[10px] duration-200 border border-[#10B8FF] text-[#10B8FF] hover:bg-[#10B8FF] hover:text-white"
                        onClick={handleAddRecordFormat}
                    >
                        <span> Add Record Format</span>
                        <img src={plusicon} alt="Add Icon" className='w-[18px] h-[18px] enms-add-plus' />
                    </button>
                </div>
            </div>
                            ...
                            {paginatedRecordFormat.length > 0 ? (
                                paginatedRecordFormat.map((recordFormats, index) => (
                                    <tr key={recordFormats.id} className="border-b border-[#383840] hover:bg-[#1a1a20] cursor-pointer h-[46px]">
                                        ...
                                        <td className="px-5 add-manual-datas">{recordFormats.title}</td>
                                        <td className="px-5 add-manual-datas">{recordFormats.no}</td>
                                        ...
                                        <td className="px-5 add-manual-datas">{recordFormats.rivision || 'N/A'}</td>
                                        <td className="px-5 add-manual-datas">{formatDate(recordFormats.date)}</td>
                                ...
                                <tr>
                                    <td colSpan="8" className="text-center py-4 not-found">No Record Formats found.</td>
                                </tr>
                            )}
                            <tr>
