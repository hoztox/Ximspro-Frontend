...
const EnMSProcedure = () => {
    ...
    const filteredProcedure = procedure.filter(procedures =>
        (procedures.title?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
        (procedures.no?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
        (procedures.approved_by?.first_name?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
        (procedures.rivision?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
        (formatDate(procedures.date)?.replace(/^0+/, '') || '').includes(searchQuery.replace(/^0+/, ''))
    );
                            ...
                            {paginatedProcedure.length > 0 ? (
                                paginatedProcedure.map((procedure, index) => (
                                    <tr key={procedure.id} className="border-b border-[#383840] hover:bg-[#1a1a20] cursor-pointer h-[46px]">
                                        ...
                                        <td className="px-5 add-manual-datas">{procedure.rivision || 'N/A'}</td>
                                        <td className="px-5 add-manual-datas">{formatDate(procedure.date)}</td>
